...
from experiments.e00.run_tests import RunAllTests  # noqa: E402
...
def GetOutputPaths(ReportsRoot: Path, ArtifactsRoot: Path) -> list[Path]:
    ReturnPaths = [
        ...
        ReportsRoot / "e00-label-mask-audit.csv",
        ReportsRoot / "e00-group-crossings.csv",
        ReportsRoot / "e00-head-metrics.csv",
        ReportsRoot / "e00-seed-summary.csv",
        ...
        ReportsRoot / "e00-runtime.json",
        ReportsRoot / "e00-run-manifest.json",
        ArtifactsRoot / "e00-run.json",
        ArtifactsRoot / "e00-test-results.json",
    ]
...
def EnsureOutputsAbsent(OutputPaths: Sequence[Path]) -> None:
    ...
    if ExistingPaths:
        raise FileExistsError(
            "E00 refuses to overwrite existing outputs:\n" + "\n".join(ExistingPaths)
        )
...
def BuildReport(
    ...
    Lines = [
        ...
        f"- validation 행: {RunRecord['split_audit']['validation_row_count']:,}",
        f"- content group: {RunRecord['split_audit']['content_group_count']:,}",
        ...
        "",
        f"전체 mask audit는 {len(MaskSummaryRows)}개 dataset-head 조합을 확인했다. source dataset별 정확한 관측·mask 수는 `e00-label-mask-audit.csv`에 있다.",
        "",
        "## 통과 조건",
        "",
...
def Execute() -> dict[str, Any]:
    ...
    ArtifactsRoot = DeepvoiceRoot / "artifacts" / "e00"
    ...
    AllRows, FieldNames = LoadManifestRows(ManifestPath)
    ValidateManifestRows(AllRows, FieldNames)
    AllLabels, AllMasks = BuildLabelMasks(AllRows)
    ManifestLoadSeconds = time.perf_counter() - ManifestLoadStarted

    Crossings, SplitSummary = AuditGroupCrossings(AllRows)
    if Crossings:
    ...
    ValidationRows = [AllRows[int(RowIndex)] for RowIndex in ValidationIndices]
    ValidationLabels = AllLabels[ValidationIndices]
    ValidationMasks = AllMasks[ValidationIndices]
    NonTestRows = [AllRows[int(RowIndex)] for RowIndex in NonTestIndices]
    NonTestLabels = AllLabels[NonTestIndices]
    NonTestMasks = AllMasks[NonTestIndices]

    Tests = RunAllTests()
    WriteJson(ArtifactsRoot / "e00-test-results.json", Tests)
    ...
    MaskSummaryRows = SummarizeLabelMasks(AllRows, AllLabels, AllMasks)

    WriteCsv(ReportsRoot / "e00-label-mask-audit.csv", MaskSummaryRows)
    WriteCsv(
        ReportsRoot / "e00-group-crossings.csv",
        Crossings
        if Crossings
        else [{"content_group_key": "", "splits": "", "row_count": 0}],
    )
    WriteCsv(ReportsRoot / "e00-head-metrics.csv", HeadMetricRows)
    WriteCsv(ReportsRoot / "e00-seed-summary.csv", BuildSeedSummary(HeadMetricRows))
    WriteCsv(
        ReportsRoot / "e00-bootstrap-summary.csv",
        MakeBootstrapSummaryRows(BootstrapSummaries),
    )
    WriteCsv(ReportsRoot / "e00-shortcut-label-audit.csv", ShortcutLabelRows)
    WriteCsv(ReportsRoot / "e00-shortcut-metric-fixture.csv", ShortcutMetricRows)
    WriteJson(ReportsRoot / "e00-singleton-equivalence.json", SingletonResults)
    ...
    RunRecord = {
        ...
        "test_split_policy": "group crossing only; no test predictions or test metrics",
    ...
    WriteJson(ReportsRoot / "e00-runtime.json", {"runtime_seconds": Runtime, "cost": Cost})
    WriteJson(ReportsRoot / "e00-run-manifest.json", RunRecord)
    WriteJson(ArtifactsRoot / "e00-run.json", RunRecord)
    ReportText = BuildReport(RunRecord, MaskSummaryRows, ShortcutLabelRows)
    ReportPath = ReportsRoot / "e00-experiment-batch.md"
