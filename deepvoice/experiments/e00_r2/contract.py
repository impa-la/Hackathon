...
CrossingColumns = frozenset(("content_group_key", "recommended_content_split"))
...
def ValidateTestCrossingRows(Rows: Sequence[dict[str, str]]) -> None:
    for Row in Rows:
        if set(Row) != CrossingColumns:
            raise ValueError("Test crossing rows must contain only group and split")
        if Row["recommended_content_split"] != "test":
            raise ValueError("Test crossing row has a non-test split")
        if not Row["content_group_key"]:
            raise ValueError("Test crossing row has an empty content group")


def LoadManifestPartitions(
    ManifestPath: Path,
) -> tuple[list[dict[str, str]], list[dict[str, str]], list[str], int]:
    with gzip.open(ManifestPath, "rt", encoding="utf-8-sig", newline="") as FileHandle:
        ...
        MissingCrossingColumns = sorted(CrossingColumns - set(FieldNames))
        if MissingCrossingColumns:
            raise ValueError(
                f"Manifest crossing columns are missing: {MissingCrossingColumns}"
            )
        NonTestRows = []
        TestCrossingRows = []
        TotalRowCount = 0
    ...
    ValidateTestCrossingRows(TestCrossingRows)
    return NonTestRows, TestCrossingRows, FieldNames, TotalRowCount


def ValidateNonTestManifestRows(
    Rows: Sequence[dict[str, str]],
    ...
    AllowedSplits = {"train", "validation"}
...
def AuditGroupCrossings(
    ...
    for Row in Rows:
        if set(Row) != CrossingColumns:
            raise ValueError("Crossing audit accepts only group and split projections")
    ...
    Summary = {"crossing_group_count": len(Crossings)}
