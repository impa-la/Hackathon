...
from .cache import LoadCachedOrRawWaveform, VerifyCompleteCache
...
from .strict_serialization import AssertFinitePayload
...
class EpochTaggedSampler(Sampler[tuple[int, int]]):
    def __init__(self, BaseSampler: GroupFirstBalancedSampler) -> None:
        self.BaseSampler = BaseSampler

    def SetEpoch(self, Epoch: int) -> None:
        self.BaseSampler.SetEpoch(Epoch)

    def __len__(self) -> int:
        return len(self.BaseSampler)

    def __iter__(self):
        Epoch = self.BaseSampler.Epoch
        return iter((Epoch, Index) for Index in self.BaseSampler)
...
class TrainingDataset(
    ...
    def __init__(
        ...
        self.CacheRoot = Path(Config["resolved_cache_root"])
    ...
    def __getitem__(
        ...
        if isinstance(TaggedIndex, tuple):
            Epoch, Index = TaggedIndex
        else:
            Epoch, Index = self.Epoch, TaggedIndex
        Record = self.Records[Index]
        Waveform = LoadCachedOrRawWaveform(
            Record,
            self.CacheRoot,
            self.SampleRate,
        )
        Offset = StableOffset(self.Seed, Epoch, Record)
        ...
        AugmentSeed = self.Seed + Epoch * 1_000_003 + Index
...
def PredictRecord(
    ...
    Waveform = LoadCachedOrRawWaveform(
        Record,
        Path(Config["resolved_cache_root"]),
    ...
    for Start in range(0, Segments.shape[0], BatchSize):
        ...
        RequireFiniteTensor(Logits, "validation", Start, "logits")
        Probabilities = torch.sigmoid(Logits)
        RequireFiniteTensor(Probabilities, "validation", Start, "probabilities")
        SegmentPredictions.append(Probabilities.cpu())
    FilePrediction = torch.cat(SegmentPredictions).mean(dim=0)
    RequireFiniteTensor(FilePrediction, "validation", 0, "file_prediction")
    return FilePrediction.numpy().astype(np.float64)
...
def RunSeed(
    ...
    for Epoch in range(int(Config["epochs"])):
        ...
        for BatchIndex, (
            ...
            RequireFiniteTensor(Waveforms, f"seed_{Seed}_epoch_{Epoch}", BatchIndex, "waveforms_cpu")
            Waveforms = Waveforms.to(Device, non_blocking=True)
            ValidSampleCounts = ValidSampleCounts.to(Device, non_blocking=True)
            Labels = Labels.to(Device, non_blocking=True)
            Masks = Masks.to(Device, non_blocking=True)
            RequireFiniteTensor(Waveforms, f"seed_{Seed}_epoch_{Epoch}", BatchIndex, "waveforms_gpu")
            RequireMaskedLabels(Labels, Masks, f"seed_{Seed}_epoch_{Epoch}", BatchIndex)
    ...
    if not np.isfinite(Predictions).all():
        raise RuntimeError(f"Seed {Seed} validation predictions are nonfinite")
    ...
    ResultPayload = {
        ...
        "singleton": Singleton,
        "numerical_integrity": NumericalSummary,
    }
    AssertFinitePayload(ResultPayload)
    CheckpointPath = DeepvoiceRoot / "checkpoints" / "e01_r4" / f"seed-{Seed}.pt"
...
def RunFullTraining(DeepvoiceRoot: Path, Config: dict[str, Any]) -> dict[str, Any]:
    ...
    if CacheGate["status"] != "PASS":
        raise RuntimeError(f"Full E01-R4 training cache gate failed: {CacheGate}")
    SeedRuns = [
